...
async function setupDemo(setupDemo) { // eslint-disable-line no-unused-vars
    ...
    supplier.email = 'supplier@email.com'
    ...
    manufacturer.email = 'manufacturer@email.com'
    ...
    retailer.email = 'retailer@email.com'
    retailer.address = retailerAddress;
    retailer.accountBalance = 0;

    // create the retailer
    const distributor = factory.newResource(NS, 'Distributor', 'distributor@email.com');
    const distributorAddress = factory.newConcept(NS, 'Address');
    distributorAddress.country = 'Panama';
    distributor.email = 'distributor@email.com'
    ...
    // contract.buyer = factory.newRelationship(NS, 'Supplier ', 'supplier@email.com');
    // contract.seller = factory.newRelationship(NS, 'Manufacturer', 'manufacturer@email.com');
    ...
    // await shipmentRegistry.addAll([shipment]);
        ...
        console.log("error in setupDemo = " + error)
        throw error;
    }
...
async function isAnyAssetBeingShipped(assets){
    ...
    for(var i = 0; i < assets.length;i++){
        ...
        const shipment = await query('getUnDeliveredShipmentWhereCommodityExists', {commodities:assetIdentifier});
